...
function update(dt,t){
  ...
  if(t>collisionLock)for(const o of ice){const od=Math.hypot(player.x-o.x,player.y-o.y);if(od<o.r+18){player.hull-=34;collisionLock=t+1200;const nx=(player.x-o.x)/od,ny=(player.y-o.y)/od;player.x=o.x+nx*(o.r+24);player.y=o.y+ny*(o.r+24);player.targetX=player.x-nx*90;player.targetY=player.y-ny*90;break}}
  updateUI();
...
function draw(t){
  ...
  ice.forEach(o=>{if(!visible(o))return;ctx.save();ctx.translate(o.x,o.y);ctx.fillStyle='rgba(166,235,237,.18)';ctx.strokeStyle='rgba(201,255,255,.8)';ctx.shadowColor='#a7ffff';ctx.shadowBlur=12;ctx.lineWidth=3;ctx.beginPath();for(let i=0;i<10;i++){const a=i/10*Math.PI*2,r=o.r*(.78+.19*Math.sin(i*4.7));const x=Math.cos(a)*r,y=Math.sin(a)*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath();ctx.fill();ctx.stroke();ctx.restore()});
