...
function flow(){
 ...
 for(let y=3;y<CONFIG.rows;y++)for(let x=0;x<CONFIG.cols;x++){const i=idx(x,y),amount=lava[i];if(amount<.025)continue;const routes=[];
  ...
  routes.sort((a,b)=>a[1]-b[1]);if(!routes.length)continue;const move=Math.min(amount*.82,CONFIG.flowStep),split=amount>.16&&routes[1]&&routes[1][1]-routes[0][1]<.22;next[i]-=move;next[routes[0][0]]+=move*(split ? .8 : 1);if(split)next[routes[1][0]]+=move*.2
 }
