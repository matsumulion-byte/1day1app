// 調整しやすいゲーム定数
const CONFIG={cols:24,rows:32,eruptionDelay:3.5,gameSeconds:32,digPower:105,digCost:1.25,digDepth:.42,flowStep:.22,lavaRate:3};
...
let campaignScores=[null,null,null];
...
function flow(){
 const next=new Float32Array(lava);const dirs=[[0,1],[-1,1],[1,1],[-1,0],[1,0]];
 for(let y=3;y<CONFIG.rows;y++)for(let x=0;x<CONFIG.cols;x++){const i=idx(x,y),amount=lava[i];if(amount<.025)continue;const routes=[];
  // 掘った低地を最優先しつつ、前線は毎回必ず前進させる。
  // 「溜まるまで停止」をなくし、ミニゲームとして気持ちよいテンポを優先する。
  for(const [dx,dy] of dirs){const nx=x+dx,ny=y+dy;if(!inGrid(nx,ny))continue;const ni=idx(nx,ny),cost=terrain[ni]+lava[ni]*.18-dy*.42;routes.push([ni,cost])}
  routes.sort((a,b)=>a[1]-b[1]);if(!routes.length)continue;const move=Math.min(amount*.46,CONFIG.flowStep),split=routes[1]&&routes[1][1]-routes[0][1]<.3;next[i]-=move;next[routes[0][0]]+=move*(split ? .76 : 1);if(split)next[routes[1][0]]+=move*.24
 }
...
function finish(){if(ended)return;ended=true;const homes=objects.filter(o=>o.type==='house'),burnedHomes=homes.filter(o=>o.burned).length,burnedTrees=objects.filter(o=>o.type==='tree'&&o.burned).length,springs=objects.filter(o=>o.type==='spring'&&!o.active).length;const damage=(burnedHomes*500+burnedTrees*50+roadHits.size*100)*10000;let score=(seaReached?1000:0)+(homes.length-burnedHomes)*300+springs*200-burnedHomes*500-burnedTrees*50-roadHits.size*100;score=Math.max(0,score);
 campaignScores[stageIndex]=score;
 setTimeout(()=>{cancelAnimationFrame(raf);const campaignComplete=stageIndex===STAGES.length-1;const displayScore=campaignComplete?campaignScores.reduce((sum,n)=>sum+(n||0),0):score;$('resultIcon').textContent=seaReached?'🌊':'🌋';$('resultTitle').textContent=campaignComplete?'全ステージ終了！':seaReached?'海まで到達！':'誘導失敗…';$('resultComment').textContent=displayScore>=5000?'火山防災マスター':score>=1800?'防災のプロ':score>=1100?'まあまあ避難できた':score>=500?'ほぼ災害':'火山に負けた';$('scoreLabel').textContent=campaignComplete?'3ステージ総合スコア':'ステージスコア';$('totalScore').textContent=displayScore.toLocaleString();$('stageScoreRow').hidden=!campaignComplete;$('stageScore').textContent=`${score.toLocaleString()} pts`;$('seaResult').textContent=seaReached?'到達':'失敗';$('resultDamage').textContent=`¥${damage.toLocaleString()}`;$('burnedHomes').textContent=`${burnedHomes} / ${homes.length}軒`;$('burnedTrees').textContent=`${burnedTrees}本`;$('hotSprings').textContent=`${springs}カ所`;$('nextButton').textContent=campaignComplete?'最初から遊ぶ　↻':'次のステージへ　→';showScreen('resultScreen')},seaReached?300:0)}

function startStage(){showScreen('gameScreen');requestAnimationFrame(buildStage)}
$('startButton').onclick=()=>{stageIndex=0;campaignScores=[null,null,null];startStage()};$('restartButton').onclick=startStage;$('retryButton').onclick=startStage;$('backButton').onclick=()=>{cancelAnimationFrame(raf);showScreen('titleScreen')};$('nextButton').onclick=()=>{if(stageIndex===STAGES.length-1)campaignScores=[null,null,null];stageIndex=(stageIndex+1)%STAGES.length;startStage()};window.addEventListener('resize',()=>state==='game'&&resizeCanvas());
