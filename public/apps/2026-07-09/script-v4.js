...
const judgeGrade = document.getElementById("judgeGrade");
...
const judgeLength = document.getElementById("judgeLength");
const judgeDrop = document.getElementById("judgeDrop");
const judgeTurns = document.getElementById("judgeTurns");
const judgeLoops = document.getElementById("judgeLoops");
...
function getFinalRating() {
  ...
  const tip = getRatingTip(metrics);
  return { grade, title, comment, score, tip, metrics };
}

function getRatingTip(metrics) {
  if (metrics.length < 420) return "もう少し長く描くと、旅感が出て点が伸びます。";
  if (metrics.drop < 120) return "高い山と深い谷を足すと、急降下判定が伸びます。";
...
function showJudgeModal() {
  ...
  judgeGrade.textContent = rating.grade;
  judgeTitle.textContent = rating.title;
  judgeScore.textContent = `${rating.score}点`;
  judgeComment.textContent = rating.comment;
  judgeLength.textContent = `${Math.round(metrics.length / 5)}m`;
  judgeDrop.textContent = `${Math.round(metrics.drop / 4)}m`;
  judgeTurns.textContent = `${Math.round(metrics.turns * 10) / 10}`;
  judgeLoops.textContent = metrics.loops > 0 ? `${metrics.loops}回` : "なし";
  judgeTip.textContent = rating.tip;
  judgeModal.classList.add("show");
  judgeModal.setAttribute("aria-hidden", "false");
}

function hideJudgeModal() {
  judgeModal.classList.remove("show");
  judgeModal.setAttribute("aria-hidden", "true");
}
...
function animate(time) {
  ...
  lastTime = time;
  const length = pathLength();
  progress += elapsed * Number(speedInput.value) * 0.42;
  if (progress >= length) {
    progress = length;
    running = false;
...
judgeClose.addEventListener("click", hideJudgeModal);
judgeRetry.addEventListener("click", clearTrack);
judgeModal.addEventListener("click", (event) => {
  if (event.target === judgeModal) hideJudgeModal();
});
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape") hideJudgeModal();
});
